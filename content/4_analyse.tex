...
Die automatisierte Bereitstellung (Kommentar: automatisiert ist auch die "Knopfdruck" Variante, besser) Mittels des Kommandozeileninterfaces kann die Bereitstellung auch in eine CI/CD-Pipeline integriert werden.
...
Im Fall des dieser Arbeit zugrunde liegenden Beispiels handelt es sich um siebzehn verschiedene VSAM\footnote{Virtual Storage Access Method, spezielle z/OS Dateiart, die schnelle I/O-Zugriffe ermöglicht.\cite{Lovelace.2013}} Dateien.
...
Bei einem Started Task Control-Job, kurz STC Job, handelt es sich um einen langlaufenden Batch Job, der mit Hilfe des \glqq START\grqq-Konsolenkommandos innerhalb von z/OS gestartet werden kann. (Kommentar: der läuft doch so lange wie das CICS aktiv ist, oder?)
...
Zu diesen Parametern zählen unter anderem der eindeutige Name der CICS-Instanz, der Speicherort der dazugehörenden CSD und die Information, ob eine Verbindung zu einem (anwendungsspezifischen) Db2 Datenbanksystem hergestellt werden soll. (Kommentar: auch MQ, oder? und anwendungsspezifisch wäre ja der Fall, wenn man hier eine DB2 Sandbox bereitstellt, korrekt?)
...
Der komplette Ablauf dauert, unter der Annahme, dass alle Beteiligten verfügbar sind, nur diese Anforderung umsetzen müssen und für die Beratung ein Arbeitstag veranschlagt wird, circa zwei Arbeitstage. (KOmmentar: haben wir irgendwo beschrieben, wie die Anwendung in das CICS kommt? Ist der Build Prozess, der stellt die Anwendungsartefakte auf die Bibliotheken, die dem CICS zugeordnet sind. Das ist ja auch etwas, das gemacht werden muss. SInd das die oben in Zeile 100 erwähnten CICS spezifischen Dateien? Und wie wird die DB2 und MQ an das CICS angebunden?)
...
Das Projektkürzel (eindeutige Kennung von DATEV Projekten), der Datenbank- und Projektname und die Projektbezeichnung müssen mit den involvierten Abteilungen besprochen werden.
...
Wird die Db2 Datenbank in Verbindung mit einem CICS verwendet, (wie im Fallbeispiel dieser Arbeit, der Rechnungsschreibung) so sind weitere manuelle Schritte des CICS Administratorenteams notwendig. 
...
Wird die Queue in Verbindung mit einem CICS verwendet, so sind weitere Abstimmungen und Maßnahmen des CICS Administratorenteams notwendig.
...
Der Start des gesamten Prozesses findet meist auf "Zuruf" statt.
...
Wird der Vergleich zu dem Bereitstellungsprozess im cloud native-Umfeld\footnote{Siehe Absatz \ref{sec:analysecloud}} gezogen, so steht dem automatisierten und dadurch effizienten cloud native Prozess, ein durch viele Absprachen langsamer und fehleranfälliger Prozess auf z/OS gegenüber.
Es ist festzustellen, dass die DATEV e.G. im Mainframe Umfeld noch weit vom DevOps-Gedanken entfernt.
(Kommentar: vlt. könnte man hier noch Unterüberschriften schreiben: Produktautonomie, Deplyment, Architektur)
...
Die Administratoren-Teams sind aber insb. für die produktiven Instanzen im Betrieb verantwortlich, die nach wie vor von Anwendungen geteilt werden.  
D.h. der CLoud-Aspekt des automatischen Deployments einer Anwendung inklusive Runtime zwischen Stages wird also nicht umgesetzt. (Kommentar: evtl etwas für den Ausblick?)
...
(Kommentar: Was jetzt kommt ist das , was letztendlich das ist, wovon man sich etwas verspricht. Das würde ich jetzt betonen.... bei welchem Problem hilft die automatisierte Provisionierung hier also. aktuell eben das isolierte Testen und das schnelle, autonome Bereitstellen der SUbsysteminstanzen)
Der Vorteil der hier untersuchten Automatisierung betrifft demnach v.a. die Entwicklungseffizienz.
(Kommentar: Bereitstellung isolierter Testumgebungen, ohne Wartezeiten und möglichst wenig manueller Interaktion ist das was man sich verspricht. Ob Releasezyklen dadurch schneller werden, kann man nciht messen, nur die AUswirkung auf die Entwicklungszyklen, bzw. weniger Fehleranfälligkeit etc.)
Um das Testen effizienter zu gestalten und einen Schritt in Richtung DevOps, einer CI/CD-Pipeline und dadurch einer Verkürzung der Releasezyklen (s. Kommentar) auch für monolithische z/OS Anwendungen zu gehen, wird mit Hilfe des \glqq IBM Cloud Provisioning and Management for z/OS\grqq-Tools der Aspekt der automatisierten Bereitstellung von Laufzeitumgebungen für die Entwicklungsstage untersucht.
...
Aufgrund dieser Last wurde die Berechnung zum einen in CICS-Instanzen ausgelagert (Kommentar: von vorher Batch, aber das weiß man ja nicht, deshalb klingt das etwas komisch. Vlt. besser: wird die Berechnung in CICS durchgeführt und in 2 Teile zerlegt... muss der Prof ja nicht wissen, dass das vorher Batch war, oder?) und zum anderen wurde der Ablauf in zwei Teile zerlegt:
\begin{itemize}
