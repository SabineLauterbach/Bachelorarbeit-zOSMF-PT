...
Um diesen Fluss zu maximieren sollten möglichst kleine Änderungen in möglichst kleinen Arbeitsabschnitten entwickelt werden.
...
Zur Erreichung dieses Ziel gehört unter anderen das Abschaffen der Schubladendenkweise.
...
Bei der DATEV e.G. äußert sich diese DevOps-Gedanke in eigenen sogenannten \glqq DevOps-Teams\grqq (STIMMT DAS????).
Ein solches Team umfasst nicht nur Mitarbeiter für die Softwareentwicklung einer Anwendung sondern auch Mitarbeiter, die sich um die Middleware der Anwendung kümmern, die Testfälle für die Anwendung zur Verfügung stellen und Fachberater.
Dem ganzen Team wird viel Vertrauen gegeben und somit kann es weitestgehend autonom, ohne viele Absprachen arbeiten.
...
Durch Continuous Delivery und Continuous Deployment kommen diese Änderungen voll automatisiert beim Kunden an.
Dies führt auch zu einer Effizienzsteigerung.

Für die Bereitstellung der benötigten Laufzeitumgebung kommt bei DATEV e.G. die Cloud Foundry Distribution von \glqq pivotal\grqq{} zum Einsatz.
Durch den zur Verfügung gestellten \glqq Marketplace\grqq{} können Laufzeitumgebungen effizient generiert werden.
Eine automatisierte Bereitstellung ist mittels des Kommandozeileninterfaces und der Aufnahme in eine CI/CD-Pipeline möglich.
So werden unter anderem isolierte Testumgebungen geschaffen.

\section{Aktueller Bereitstellungsprozess}\label{sec:aktbereit}
Um einen Vergleich zum automatisierten Bereitstellungsprozesses im cloud native-Umfeld ziehen zu können, wird im Folgenden der aktuell etablierte Prozess beschrieben.
