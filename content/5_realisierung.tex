...
Die Bereitstellung einer Instanz ist wiederholbar, so kann die Instanz z.B. im Fehlerfall sicher neu erstellt werden und muss nicht langlebig gepflegt werden.(Kommentar: erstellt weil "neu gestartet" einen Neustart von CICS impliziert, nicht den eines Templates zur neu ERstellung einer Instanz)
...
Diese müssen in eine externe Konfigurationsdatei als Umgebungsvariablen aufgenommen werden. (Kommentar: die Aussage fnide ich nah wie vor schwierig. Um ehrlich zu sein klingt das ziemlich lapidar, dann mach ich das halt... so richtig erschließt sich mir das "nicht ohne weiteres möglich" nicht)
...
Schließlich konnte, wie im Kapitel \ref{ch:vorgehensweise} beschrieben, mit dem ersten Versuch, das bei der Installation von z/OSMF mitgelieferte \glqq cics\_getting\_started\grqq{} Template zu provisionieren, begonnen werden.
...
Im Falle einer gemeinsamen (Kommentar: extra ist umgangssprachlich) CSD Datei für alle provisionierten Instanzen müssten Änderungen, wie beispielsweise die Verfügbarkeit einer neueren CICS Version, an mehreren Stellen angepasst werden. (Kommentar: ist das so? Klingt eher danach, dass ich solche Änderungen durch die Kopierereie erst recht in mehreren INstanzen anpassen muss... ist missverständlich)
...
Für das Einbinden der benötigten Bibliotheken und der zuvor angelegten CICS spezifischen Dateien ist nur das Hinzufügen weiterer DD-Statements notwendig. (Kommentar
...
In der Systemumgebung Testplex bedeutet dies die Provisionierung der Datenbank (Instanz??) ohne Tabelle und Daten.
...
Dieser Aufruf wurde mittels neuer Variablen im Template möglichst dynamisch gestaltet und mussten in dem Variableinputfile gesetzt werden.
...
Dies würde bedeuten, dass alle Anwender dieses Templates diese Rechte auch benötigen würden.
...
Für Db2 ist ein solcher User nicht notwendig, da das Datenbanksystem hinter der REST-API für alle zugänglich ist und jeder darauf Datenbanken erstellen darf. (Kommentar: das ist aber nur so, weil es sich um die DB2 Sandbox handelt, oder? Ist das was, was man adressieren sollte? Schließlich ist das Rest Interface ja auch eine Art Provisionierung, aber eben nur für ein einzelnes DB2 nicht die komplette Umgebung)
...
Dafür bietet z/OSMF die Möglichkeit, beim Zuweisen des \glqq Tenants\grqq{} eine Standard Jobkarte (Kommentar: Link?), die vor jeden Job des Templates eingefügt wird, zu hinterlegen.
...
Dahingehend wurde die \glqq createCICS.jcl\grqq-Datei angepasst. (Kommentar: COdeabschnitt??)
...
Eine weitere CSD Gruppe musste hinzugefügt werden. (Kommentar: Codeabschnitt?)
...
(Kommentar: das ist ein wichtiger Punkt, ich hab das schon an anderer Stelle erwähnt. Hier wird erstmals darauf hingewiesen, wie die Anwendung in die bereitgestellte Umgebung kommt, dass das das Ergebnis eines Builds ist, der die ausführbaren Module in eine Bibliothek stellt, die dann  der Umgebung zugeordnet ist. Das weiß man nicht "automatisch", würde ich vlt. als Grafik machen) 
Dies ist ein Standardvorgehen innerhalb der DATEV e.G., mit dem es möglich ist, neue Programmversionen, und Schnittstellen zu bereits produktiven Modulen nutzen zu können.

\subsection{Db2 Anpassung}\label{ssec:db2entw}
Eine genauere technische Analyse der DATEV-Rechnungsschreibungsdatenbank kam zu dem  Ergebnis, dass es zwar möglich wäre, diese Datenbank für Testzwecke zu provisionieren, die Erstellung des dafür notwendigen Templates (Kommentar: mit DDLs usw. oder? Was genau muss hierfür gemacht werden? Das ist zu unspezifisch und stellt das Konzept in Frage) aber den zeitlichen Rahmen dieser Arbeit übersteigen würde.
...
(Kommentar: was genau? .. siehe oben, das kommt hier nicht raus)
...
Im weiteren LEbenszyklus der Anwendung kann auf diese DB2 jederzeit zugegriffen werden, Änderungen sind ein erheblich geringerer Aufwand als die Initialerstellung.

Für die weiteren Schritte der hier vorliegenden Arbeit werden deshalb Datenbanken eines bereits existierenden Datenbanksystems für die DATEV e.G. Rechnungsschreibung genutzt.
...
Außerdem wurden sowohl in der Provisionierungs- als auch in der Deprovisionierungsdatei die Datenbanksteps auskommentiert, da diese für den weiteren Test nicht mehr zum Einsatz kommen.
...
Für jeden IBM MQ bezogenen Job wurde zuallerletzt die Jobkarte angepasst und der technische User der CICS-Administration durch den technischen User der IBM MQ-Administration, der für administrative Aufgaben berechtigt ist, ausgetauscht.(Kommentar: ist das relevant??)
...
Als Eingabe dienen vordefinierte Dateien, die Ergebnisse werden ebenfalls in Dateien geschrieben.
...
(Kommentar: sind das wirklcih JObs oder Steps?)
...
Anschließend wurden die Ausgabedateien beider Läufe verglichen. (Kommentar: und wie war das Ergebnis??)
...
(Kommentar, das sind die technischen Use-Cases, aber in welchen fachlichen könnten die notwendig sein?)
...
(Kommentar: welchen fachlichen Usecase könnte das abdecken? Wann braucht man eine  neue Instanz?)
...
Außerdem besteht eine Art von String Verarbeitung.(Kommentar: steht etwas sehr im Luftleeren Raum. Was meinst du damit, für was ist es gut?)
Wenn am Zeilenanfang ein \glqq\#\grqq{} steht, kann die dahinter angegebene Programmiersprache verwendet werden. (Kommentar: z.B. REXX?)
...
Zu sehen ist zunächst eine \glqq set\grqq{} Anweisung, mit der Variablen zugewiesen werden können, If-Bedingungen und eine foreach-Schleife stehen außerdem zur Verfügung. (Kommentar: was willst du damit sagen? Dass es eine Sprache gibt, die man nicht kennt, in der es aber set, if und foreach gibt? Wofür steht dann die Programmiersprache hinter dem #?)
...
Somit liegt dem Wissen über diese Sprache nur der Code aus Beispielen der IBM zugrunde. (Kommentar: hast du da bei Marcel mal nachgefragt?)
...
Die Aufnahme in eine solche Gruppe, um zum Beispiel die z/OSMF Oberfläche nutzen zu dürfen, geschieht auf Zuruf und manuelles Hinzufügen einer User ID durch einen Mitarbeiter (Kommentar: in der RACF-Administration??).
...
Damit würde das Gewähren der expliziten Rechte zum Starten eines Jobs mit der technischen User Id entfallen und damit die manuelle Arbeit des \glqq Gewährens\grqq, mittels eines Formulars.(Kommentar das was auf Zeile 501 genannt ist? Da steht nix von FOrumular)
...
Dieser würde sicherstellen, dass nur die für ein Template zugelassenen User dieses Template auch provisionieren dürfen. (Kommentar: d.h. dann dürften nur Rechnungsschreibung Entwicler das Rechnungsschreibung Template provisionieren??)
...
Dadurch ist es notwendig, die nicht optimale Lösung mit der Jobkarte (s.o.) einzusetzen.
