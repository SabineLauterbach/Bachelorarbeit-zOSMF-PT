...
In diesem Kapitel wird beschrieben, wie bei der Beantwortung der Forschungs-Fragen der Arbeit (Kommentar: Link zur EInleitung) vorgegangen wurde.
Dazu wird nach der im Kapitel \ref{ch:vorgehensweise} beschriebenen Reihenfolge der Arbeitsschritte vorgegangen.
Es ist noch einmal zu erwähnen, dass zunächst die Provisionierung einer CICS-Instanz untersucht wird.
...
\section{Testplex}
Der Zugriff auf Ressourcen und Tools bei DATEV e.G. wird über ein Rechtekonzept über RACF (Kommentar: Link) verwaltet. Um die Forschungsfragen beantworten zu können, mussten vor Beginn der eigentlichen Untersuchung zunächst alle benötigten Rechte beantragt werden.
...
Beispielsweise benötigt das \glqq IBM Cloud and Management for z/OS\grqq-Toolkit lesenden Zugriff auf den Speicherpfad der Template Dateien.
Deshalb wurde zu Beginn der Umsetzung auf dem Testplex gearbeitet.
Hier ist es  möglich, die Rechte für das Erstellen der CICS Dateien, das Starten einer CICS-Instanz und die Administration von Db2 und IBM MQ einer persönlichen UserID zu geben, was auf dem eigentlichen Entwicklungs-Plex nicht ohne weiteres umsetzbar ist ist. 

Damit eignet sich der Testplex - als reine Systemtestumgebung - gut für die ersten Versuche mit der Implementierung der Provisionierung.

Schließlich konnte mit dem ersten Versuch, ein bei der Installation von z/OSMF mitgeliefertes minimales CICS Template zu provisionieren, begonnen werden.  (Kommentar: unterschiede zwischen den verprobten Templates irgendwo beschreiben, das "minimale", das umfangreichere, das an DATEV angepasste - was ist da spezifisch, usw.)
...
Trotz der Vorteile durch die Weboberfläche von z/OSMF (Kommentar: Link auf Kap)  wurde zunächst auf z/OSPT gesetzt.
...
Jedoch zeigte sich ein weiterer Nachteil des Kommandozeileninterfaces, es ist nicht möglich Templates eine Domain und einen Tenant zuzuweisen. (Kommentar: warum brauche ich das? wie löse ich das langfristig? Ist das ein KO Kriterium generell für zOSPT usw.)
Dies hatte zur Folge, dass der Befehl \glqq zospt build\grqq{} fehlschlug.
Für alle folgenden Aufgaben wurde deshalb z/OSMF genutzt. (Kommentar: siehe meine Bemerkungen in der Mail. DIe Entscheidung für die Alternative wrüde ich ausführlicher ableiten)

(Kommentar: hier würde ich auf Kap. 2.4.2 verweisen... wie in Kap. 2.4.2 aufgezeigt muss das Template in die Software Services.... )
Um das Template in die Software Services\footnote{Beschreibung in Absatz \ref{sssec:zosmf}} von z/OSMF aufzunehmen, müssen die Template-und Workflow-Dateien in einem Unix Dateisystem auf dem Großrechner abgelegt sein. 
Dabei wurden ihm eine Domain und ein Tenant zugewiesen.
Bevor das Template provisioniert werden kann (Kommentar: was bedeutet hier die "Provisionierung eines Templates"; ich will ja ein CICS provisionieren...) , müssen Änderungen in der Variableinputfile vorgenommen werden. (Kommentar: warum... weil es nicht zur DATEV Umgebung passt? dann muss man das hier erwähnen...  - auch dass es sich um eine Art Konfigurationsfile handelt, in dem man das Template an die spezifischen Anforderungen der Firma handelt.) 
Dazu mussten die Werte, der in der Tabelle \ref{tab:cgsvars} genannten Variablen angepasst werden.
Die Kurzbeschreibungen und die Beschreibungen aller Variablen, die im Standard Template vorhanden sind, ist in \cite{IBM.2019} zu finden. (Kommentar, das ist gut, aber der Sinn des Variableninputfiles kommt nicht raus)
\begin{table}[h]
\centering
...
\label{tab:cgsvars}
\end{table}
Mit Hilfe der z/OSMF Oberfläche konnte das Template aktualisiert und somit die Änderungen übernommen werden.
...
Das Ziel, die technische Umsetzung bei DATEV e.G. grundsätzlich zu verproben, war damit erfüllt.

\subsubsection{DATEV e.G. spezifischen CICS Template}\label{sssec:datevcics}
Nachdem das minimale IBM Standard CICS Template funktionsfähig war und erste Erfahrungen mit z/OSMF gesammelt worden waren, wurde ein allgemeines, mitgeliefertes Template untersucht. (Kommentar: von wem mitgeliefert, wodurch unterscheidet es sich vom oben genutzten Template usw.)
Ziel dieses nächsten Schritts war die Provisionierung einer funktionsfähigen DATEV e.G. spezifischen CICS-Instanz. (Kommentar: was ist der Unterschied zu dem ersten Schritt? Das kommt nicht rüber. du könntest in der vorhergehenden Section sagen, warum uns das für die DATEV nicht weiterhilft)
...
Die Analyse ergab, dass das mitgelieferte Template mit insgesamt 76 verwendeten Dateien sehr komplex und umfangreich ist.
...
Zu diesen unterschiedlichen Konfigurationen zählt unter anderem die Unterscheidung, ob die CICS-Instanz einem Sysplex (Kommentar weiß jemand, was das ist? ist das wichtig? Würde den Satz entfernen) hinzugefügt werden soll oder nicht.
Die Übersichtlichkeit des allgeminen Templates ist dadurch geringer. (Kommentar: als was? und ist Übersichtlichkeit ein wichtiges Kriterium?)
Es war also nötig, dieses umfangreiche Template zu überarbeiten und an DATEV spezifische Anforderungen anzupassen. 
Dazu gehörte das Entfernen von allen für ein DATEV e.G. spezifischen CICS nicht notwendigen Dateien, sowie nicht benötigte Variablen und Steps.
Wie in der Tabelle \ref{tab:vglTemps} zu sehen ist, konnten dadurch circa die Hälfte der Dateien gelöscht werden und bei der provision.xml wurde ein Drittel an Quellcode eingespart werden. (Kommentar: ist Umfang reduzieren das Ziel? doch eher nciht. Ziel war es , ein zielgerichtetes, auf DATEV spezifische Anforderungen angepasstes Template zu erhalten, das musst beschrieben werden.)
...
(Kommentar: ich würde die Schritte als Problem/Ziel/Umsetzung/Fazit aufbauen. 
WIe beschrieben benötigt eine CICS Instanz enie sog. CSD (Link auf 2.3.1)-Datei, die die notwendigen Ressourdcen definiert. 
Als nächster Schritt wurde in Zusammenarbeit mit den CICS Administratorenteam diskutiert, wie im Umfeld der automatisierten Bereitstellung die CSD Dateien verwaltet werden sollen. 
Es wurde festgelegt, dass jede provisionierte CICS-Instanz ihre eigene CSD Datei zur Verfügung gestellt bekommen soll. 
Hierfür soll die bestehende, von den Kollegen gepflegte Datei automatisiert (Kommentar: ist doch so, im Batch, s.u.?)  kopiert und mit bestimmten Namenkonventionen gespeichert werden.
...
Das ermöglicht auch jedem Nutzer der Provisionierung (wer wäre das? der Admin? derEntwickler? Noermalerweise hat ein ENtwickler damit nichts zu tun ==> klarstellen), die CSD seiner eigenen CICS-Instanz ohne Seiteneffekte zu bearbeiten.
...
Dieser Job wurde als neuer Steps in den z/OSMF Workflow eingebunden.
Innerhalb der CSD mussten bestimmte Gruppen zu der CSD Liste der CICS-Instanz hinzugefügt werden. (Kommentar: warum?)
Die JCL ist in Abbildung \ref{code:addCSD} dargestellt.
Die Reihenfolge ist relevant, da sie der Initialisierungsreihenfolge (Kommentar: was ist die Initialisierung?)  entspricht.
...
Im Standard IBM Template beinhaltet diese zunächst ein Makro für die Validierung der SIT Parameter. (Kommentar: warum "zunächst"? Wenn du das schreibst wartet man auf ein "später")
...
Sowohl die Einbindung der benötigten Bibliotheken, als auch das Einbinden der zuvor angelegten CICS spezifischen Dateien beschränkte sich auf das Hinzufügen weiterer DD-Statements. (Kommentar: irgendwie ergibt der Satz keinen Sinn...)
...
Nach und nach wurde damit die für die DATEV eG spezifische CICS Provisionierung notwendige Logik, (siehe Abbildung \ref{gleiche abbildung wie oben}),  hinzugefügt .
...
Darüber hinaus wurde erfolgreich nachgewiesen, dass Standard Transaktionen der DATEV eG  in dieser Instanz funktionsfähig sind.
Die Deprovisionierung verlief nach Plan. (Kommentar: ein wichtiger Punkt in automtisierten Prozessen ist eben das schnelle Provisioniieren als auch Deprovisionieren von Instanzen. Das könnte man hier erwähnen. Dass man eben nicht CICS Instanzen langlebig  hegen und pflegen muss, sondern dass sie automatisiert, verlässlich ohne manuelle Abstimmung immer wieder erstellt werden können.
