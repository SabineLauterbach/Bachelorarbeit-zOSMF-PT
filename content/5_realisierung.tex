...
Es ist noch einmal zu erwähnen, dass zunächst abzuwägen ist, welche Implementierungsvariante des „IBM Cloud Provisioning and Management for z/OS“ besser geeignet ist: z/OSPT oder z/OSMF.
...
Diese müssen in eine externe Konfigurationsdatei als Umgebungsvariablen aufgenommen werden. (Kommentar: ist das so schwierig,dass wir deshalb diese Lösung nicht weiter verfolgt haben? Klingt eigentlich ziemlich simpel. .. würde mich nicht überzeugen.


Diese beiden Gründe wurden als ausschlaggebend erachtet, um z/OSMF für die Beantwortung der Forschungsfragen dieser Arbeit heranzuziehen.
\begin{table}
\centering
...
Nach dem Aufnehmen werden dem Template noch eine Domain und ein Tenant zugewiesen. (Kommentar: woher kommen Domain und Tenant? wer hat die "bestimmt"? Wie sieht die Zuweisung aus?
...
\label{tab:cgsvars}
\end{table}
Mit Hilfe der z/OSMF Oberfläche konnte das Template aktualisiert und somit die Änderungen übernommen werden. (Kommentar: klingt nach "hab ich einen Save gemacht", ist das nicht etwas zu sehr basic??)
...
Nach den notwendigen Anpassungen funktionierte das Provisonieren und die definierten Aktionen des \glqq cics\_getting\_started \grqq-Templates. (Kommentar: wie wurde das geprüft? Was war das Ergebnis? Ein CICS das man bei DATEV nicht nutzen konnte - war auch nicht das Ziel, weil es hier nur um das Handling in z/OSMF mit den Templates ging, oder ? ....)
...
Es mussten neben den in Tabelle \ref{tab:cgsvars} genannten Variablen noch folgende gesetzt werden. (Kommentar: welche folgenden?? meinst du Tabelle 5.3? 
...
(Kommentar: meinst du so was wie xmon usw? das würde ich hier erwähnen, sonst kann man sich nicht so richtig vorstellen, was der Unterschied zu dem IBM Standard-CICS ist.)
Dabei handelt es sich um interne Management-Transaktionen, mit denen z.B. Entwickler ihre fachlichen Transaktionen verwalten. Beispielhaft sei hier die Transaktion XMON zu erwähnen, ein DATEV proprietäres Auskunftssystem über CICS-Transaktionen, Programme, Dateien usw.) Dies ist unabhängig von einem automatisiert provisionierten CICS-System auch weiterhin notwendig.
...
So werden Änderungen, wie beispielsweise die Verfügbarkeit einer neueren CICS Version, an alle neu provisionierten Instanzen weitergegeben. (Kommentar:das ist ein Vorteil des Kopierens? versteh ich grad nicht)
...
Ein weiterer Vorteil ist, dass bei der Deprovisionierung der CICS-Instanz diese Kopie der Standard Datei ohne Nebenwirkungen gelöscht werden kann. (Kommentar: macht das Sinn? Ist die nicht für ein wiederholtes Provisionieren notwendig?)

Um dies umzusetzen, musste ein JCL Job (Kommentar: Job oder Step?) geschrieben und in das Template eingebunden (Kommentar: korrekt?) werden, der den Kopiervorgang implementiert.
Innerhalb der CSD mussten bestimmte Gruppen, Zeilen sieben, neun und zehn in Abbildung \ref{code:addCSD}, zu der CSD Liste der CICS Instanz hinzugefügt werden. (Kommentar: das hat aber nichst mehr mit dem Kopieren zu tun, oder? liest sich so)
