...
Im Falle einer gemeinsamen CSD Datei für alle provisionierten Instanzen müssten Änderungen, wie beispielsweise die Verfügbarkeit einer neueren CICS Version, an einer weiteren Stellen angepasst werden.
...
Für Db2 ist ein solcher User nicht notwendig, da es sich beim Datenbankmanagementsystem hinter der REST-API um DB0C\footnote{Siehe Absatz \ref{sssec:db2}} handelt und jeder Entwickler in dieser \glqq Sandbox\grqq{} Datenbanken verwalten darf. 
...
Um die Datenbankstruktur der DATEV-Rechnungsschreibung im DB0C Datenbankmanagementsystem nach zu stellen, wurde eine genaue tchnische Analyse der DATEV-Rechnungsschreibungsdatenbank durchgeführt.
...
Dennoch wurde von der Db2 Administration mit der Erstellung der dafür notwendigen \Gls{ddl} Skripte begonnen.
Es stellte sich, dass bereits für einen kleinen Teil an Tabellen circa 600 Zeilen DDL Code\footnote{Im Anhang \ref{app:ddl} zufinden} notwendig ist.
...
Für die weiteren Schritte der hier vorliegenden Arbeit werden deshalb die bereits im Datenbankmanagementsystem DB0T vorhanden Datenbanken der DATEV-Rechnungsschreibung genutzt.
...
Bei einem anschließenden vergleich der Ausgabedateien beider Läufe waren keine Abweichungen festzustellen.
...
Ein Entwickler möchte einen Programmänderung auf einer isolierten Laufzeitumgebung testen.
Dadurch wird eine neue Instanz eines Templates benötigt.
Dem Entwicklerteam steht das dieses Template in z/OSMF zur Verfügung und es wurde noch keine Instanz dieses Templates provisioniert.

\item Use-Case: Zusätzliche Template Instanz\\
Ein weiterer Entwickler möchte eine andere Programmänderung auf einer isolierten Laufzeitumgebung testen.
Dadurch wird eine neue Instanz eines Templates benötigt.
Dem Entwicklerteam steht dieses Template in z/OSMF zur Verfügung aber es steht bereits eine Instanz dieses Templates zur Verfügung.
Um diese Instanz nicht zu stören, wird eine weitere Instanz benötigt.

\item Use-Case: Änderungen durch Administratorenteam\\
Die Provisionierung von einer IBM MQ Instanz soll von REXX Skripte auf eine REST-API umgestellt werden.
