...
Außerdem besteht keine Herstellerabhängigkeit.
Bei der DATEV e.G. wird auf \glqq Platform-as-a-Service\grqq{} als Ausprägung des Cloud-Computing-Konzepts gesetzt.
\cite{cloudnative.23.2.2020}

\section{\glqq Platform-as-a-Service\grqq{} bei DATEV e.G.}\label{sec:paas}
Platform-as-a-Service, kurs PaaS, beschreibt neben Insfrastructure-as-a-Service, kurz IaaS und Software-as-a-Service, kurz SaaS, eine der drei grundsätzlichen Ausprägungen des Cloud-Computing-Konzepts.
Alle drei beschreiben einen Grad an Auslagerung von IT-Systemen in die Cloud.
...
Dadurch können sich Entwickler auf die eigentliche Implementierung der Anwendung konzentrieren.
Des Weiteren ist es möglich beispielsweise Betriebssysteme oder Datenbanken auszutauschen und zu testen.
\cite{paas.23.2.2020}

Bei der DATEV e.G. kommt die sogenannte \glqq Self-Service-Cloud\grqq-Variante des PaaS-Ansatzes zum Einsatz.
Das bedeutet, dass die oben genannte Bereitstellungsplattform in einer \glqq Private Cloud\grqq{} zur Verfügung gestellt wird.
Es wird von einer \glqq Private Cloud\grqq{} gesprochen, wenn die Cloud-Umgebung unternehmensintern gehostet wird.\footnote{\cite{privatecloud.23.2.2020}}
Die Plattform stellt ein webbasiertes Portal, beispielsweise siehe Abbildung \ref{fig:marketplace}, für das Provisionieren von Infrastruktur, Betriebssystemen o.ä. zur Verfügung.
So ist es eine Bereitstellung mit wenigen Klicks möglich.
Dadurch entfällt der Umweg über die Administratorenteams.
\cite{selfservice.23.2.2020}

Die DATEV e.G. verwendet als Bereitstellungsplattform die pivotal Distribution der quelloffenen PaaS \glqq Cloud Foundry\grqq.
\glqq Cloud Foundry\grqq{} bietet zur Verwaltung von Instanzen neben einer Weboberfläche auch ein Kommandozeileninterface an.
Ein weiterer Vorteil ist, dass es sich komplett um das Staging von Anwendungen kümmert.
\cite{foundry.23.2.2020}

Durch dieses Kommandozeileninterface ist es möglich den Entwicklungsprozess im Hinblick auf das Deployment weiter zu automatisieren.
Hierbei setzt die DATEV e.G. auf Jenkins basierende \glqq Continuous Integration, Continuous Deployment\grqq-Pipeline.

\section{\glqq Continuous Integration, Continuous Deployment\grqq}
